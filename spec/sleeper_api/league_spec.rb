...
RSpec.describe SleeperApi::League do
  ...
  before do
    allow(client).to receive(:get_league).and_return(league_data)
    allow(client).to receive(:get_league_rosters).and_return(rosters_data)
    allow(client).to receive(:get_league_users).and_return(users_data)
  end
  ...
  describe "#initialize" do
    ...
    context "with no_data flag" do
      it "does not fetch league data" do
        league = described_class.new(league_id, client, no_data: true)
